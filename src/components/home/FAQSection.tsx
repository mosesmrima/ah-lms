...
const FAQItem = ({ question, answer }: FAQItemProps) => {
  ...
  return (
    <div className="border-b border-gray-800 py-4">
      <button
        className="flex items-center w-full space-x-4 text-left"
        onClick={() => setIsOpen(!isOpen)}
      >
        {!isOpen && (
          <span className="flex items-center justify-center w-6 h-6 bg-red-600 rounded-sm text-white font-bold text-sm">+</span>
        )}
        {isOpen && (
          <span className="flex items-center justify-center w-6 h-6 bg-red-600 rounded-sm text-white font-bold text-sm">-</span>
        )}
      ...
      {isOpen && (
        <div className="mt-3 text-gray-400 pr-4">
          <p>{answer}</p>
        </div>
      )}
    </div>
  ...
  return (
    <section className="py-16 px-6 md:px-12 bg-black text-white">
      <div className="container mx-auto">
        <h2 className="text-4xl font-light mb-12">Frequently Asked <span className="font-bold">Questions</span></h2>
